...
* Provide a few names formats, such as:
    PYDICT: '    "%u":  "%N",'
...
class CharInfo:
    ...
    @property
    def JARGON(self):
        if (self.c in unixJargon): return unixJargon[literal]
        return None
# pylint: disable=W0201
    def setCharInfo(self):
        ...
        #
        self.URIOK = False
        self.URIOK = n<128 and re.match(CharDisplay.okInUriExpr, c)
        self.BIDI = unicodedata.bidirectional(self.c)
        self.COMBINING = unicodedata.combining(self.c)
        self.EAWIDTH = unicodedata.east_asian_width(self.c)
        self.MIRROR = unicodedata.mirrored(self.c)
...
def loadData(self):
    ...
    if (not os.path.exists(path)):
        fatal("Could not download data from '%s'." % (self.sourceUrl))
    ...
    for charEl in charList.childNodes:
        ...
        for propEl in charEl.childNodes:
            if propEl.nodeType != Node.ELEMENT_NODE: continue
...
def strfchr(n, fmt:str) -> str:
    """Make something from one character or code point, kind of like strftime().
...
def showCodes():
    ...
    print("  Name:  Literal          P/F Calc Seb  Type Example")
...
if __name__ == "__main__":
    ...
    for rec in sys.stdin.readlines():
        for i, c in enumerate(rec):
            n0 = ord(c)
            print("  %2d: U+%04x '%s': %s" % (i, n0, c, strfchr(n0, fmt=args.format)))
